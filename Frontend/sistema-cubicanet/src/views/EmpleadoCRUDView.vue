...
const obtenerEstadosCiviles = async () => {
  const res = await axios.get(getApiUrl('/estados_civil'))
  estadosCiviles.value = res.data
}

const registrarEmpleado = async () => {
  try {
    // Asegura que estado_civil_id sea número
    const payload = { ...form.value, estado_civil_id: Number(form.value.estado_civil_id) }
    await axios.post(getApiUrl('/empleados'), payload)
